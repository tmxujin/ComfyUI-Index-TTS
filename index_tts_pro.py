...
class IndexTTSProMultiChannelNode:
    """
    ComfyUI的IndexTTS Pro多声道节点，专用于小说阅读，支持多角色语音合成并输出独立声道
    """
    ...
    # 修改返回类型，增加5个角色声道输出
    RETURN_TYPES = ("AUDIO", "AUDIO", "AUDIO", "AUDIO", "AUDIO", "AUDIO", "AUDIO", "INT", "STRING", "STRING",)
    RETURN_NAMES = ("combined_audio", "narrator_audio_out", "character1_audio_out", "character2_audio_out", "character3_audio_out", "character4_audio_out", "character5_audio_out", "seed", "Subtitle", "SimplifiedSubtitle",)
    FUNCTION = "generate_multi_channel_speech"
    CATEGORY = "audio"
    ...
    def __init__(self):
        ...
        print(f"[IndexTTS Pro MultiChannel] 初始化节点，可用模型版本: {list(self.model_versions.keys())}")
        print(f"[IndexTTS Pro MultiChannel] 默认模型目录: {self.model_dir}")
    ...
    def _init_model(self, model_version="Index-TTS"):
        ...
        if self.tts_model is None or self.current_version != model_version:
            ...
            if model_version in self.model_versions:
                self.current_version = model_version
                self.model_dir = self.model_versions[model_version]
                print(f"[IndexTTS Pro MultiChannel] 切换到模型版本: {model_version}, 目录: {self.model_dir}")
            else:
                print(f"[IndexTTS Pro MultiChannel] 警告: 未知模型版本 {model_version}，使用默认版本 {self.current_version}")
            
            # 如果已有模型，先释放资源
            if self.tts_model is not None:
                print(f"[IndexTTS Pro MultiChannel] 卸载现有模型...")
                self.tts_model = None
                # 强制垃圾回收
                import gc
                gc.collect()
            ...
            print(f"[IndexTTS Pro MultiChannel] 开始加载模型版本: {self.current_version}...")
            # 检查必要的模型文件
            required_files = ["gpt.pth", "config.yaml"]
            missing_files = []
            for file in required_files:
                ...
                if not os.path.exists(file_path):
                    ...
                    file_size = os.path.getsize(file_path) / (1024*1024)  # 转换为MB
                    print(f"[IndexTTS Pro MultiChannel] 找到模型文件: {file} ({file_size:.2f}MB)")
            
            if missing_files:
                error_msg = f"模型 {self.current_version} 缺少必要的文件: {', '.join(missing_files)}"
                print(f"[IndexTTS Pro MultiChannel] 错误: {error_msg}")
                raise FileNotFoundError(error_msg)
            ...
            try:
                ...
                # 记录加载完成时间
                load_time = time.time() - start_time
                print(f"[IndexTTS Pro MultiChannel] 模型 {self.current_version} 已成功加载，耗时: {load_time:.2f}秒")
            except Exception as e:
                import traceback
                print(f"[IndexTTS Pro MultiChannel] 初始化模型 {self.current_version} 失败: {e}")
                print(f"[IndexTTS Pro MultiChannel] 错误详情:")
                traceback.print_exc()
                raise RuntimeError(f"初始化IndexTTS模型 {self.current_version} 失败: {e}")
    ...
    def _pad_audio_to_length(self, audio_data, target_length):
        """将音频数据填充到指定长度
        ...
        if audio_data.ndim == 1:
            ...
            if current_length < target_length:
                # 用静音填充
                padding_shape = list(audio_data.shape)
    ...
    def _mix_audio_channels(self, channel_audios, sample_rate):
        """混合多个音频声道
        
        Args:
            channel_audios: 音频声道字典 {role: (audio_data, sample_rate)}
            sample_rate: 采样率
            
        Returns:
            tuple: (mixed_audio, sample_rate)
        """
        ...
        # 找到最长的音频长度
        max_length = 0
        for audio_data, _ in channel_audios.values():
            if audio_data.ndim == 1:
        ...
        # 创建多声道数组 [channels, samples]
        num_channels = len(channel_audios)
        mixed_audio = np.zeros((num_channels, max_length))
        
        # 填充每个声道
        for i, (audio_data, _) in enumerate(channel_audios.values()):
            # 将音频填充或裁剪到统一长度
            padded_audio = self._pad_audio_to_length(audio_data, max_length)
            if padded_audio.ndim == 1:
                mixed_audio[i, :] = padded_audio
            else:
                # 如果是多声道，取平均值
                if padded_audio.ndim == 2:
                    mixed_audio[i, :] = np.mean(padded_audio, axis=0)
    ...
    def _create_silence(self, duration, sample_rate):
        """创建指定时长的静音音频
        ...
        Returns:
            numpy.ndarray: 静音音频数据
        """
        num_samples = int(duration * sample_rate)
        return np.zeros(num_samples)
    
    def generate_multi_channel_speech(self, structured_text, narrator_audio, model_version="Index-TTS", 
                                   language="auto", speed=1.0, seed=0, 
                                   character1_audio=None, character2_audio=None, character3_audio=None, 
                                   character4_audio=None, character5_audio=None,
                                   temperature=1.0, top_p=0.8, top_k=30, 
        ...
        """
        try:
            print(f"[IndexTTS Pro MultiChannel] Starting multi-channel generation with structured_text: {structured_text[:100]}...")
            ...
            # 解析结构化文本
            parsed_text = self._parse_structured_text(structured_text)
            print(f"[IndexTTS Pro MultiChannel] Parsed text segments: {len(parsed_text)}")
            ...
            for role, text in parsed_text:
                print(f"[IndexTTS Pro MultiChannel] Processing: {role} - {text[:50]}...")
                ...
                if role == "Narrator":
                    ...
                    # 使用旁白音频作为默认参考
                    ref_audio = narrator_audio
                    print(f"[IndexTTS Pro MultiChannel] Warning: No specific audio for {role}, using narrator audio")
                ...
                try:
                    ...
                    if isinstance(result, tuple) and len(result) == 2:
                        ...
                        print(f"[IndexTTS Pro MultiChannel] Audio data type: {type(audio_data)}")
                        if hasattr(audio_data, 'shape'):
                            print(f"[IndexTTS Pro MultiChannel] Audio data shape: {audio_data.shape}")
                        
                        print(f"[IndexTTS Pro MultiChannel] Sample rate: {sample_rate}, type: {type(sample_rate)}")
                        ...
                        if isinstance(audio_data, np.ndarray) and audio_data.size > 0:
                            ...
                            segment_timing.append({
                                "role": role,
                                "start": current_time,
                                "end": current_time + audio_length,
                                "audio_data": audio_data
                            })
                            
                            # 添加到对应角色的音频段落列表
                            role_audio_segments[role].append((audio_data, sample_rate))
                            
                            current_time += audio_length
                        else:
                            print(f"[IndexTTS Pro MultiChannel] Warning: Invalid audio data")
                    else:
                        print(f"[IndexTTS Pro MultiChannel] Warning: Unexpected TTS model return format: {type(result)}")
                        if isinstance(result, np.ndarray):
                            print(f"[IndexTTS Pro MultiChannel] Assuming default sample rate for numpy array return: shape={result.shape}")
                            if result.size > 0:
                                audio_length = len(result) / 24000
                                start_time = self._seconds_to_time_format(current_time)
                                end_time = self._seconds_to_time_format(current_time + audio_length)
                                ...
                                role_audio_segments[role].append((result, 24000))
                                current_time += audio_length
                        elif isinstance(result, int):
                            print(f"[IndexTTS Pro MultiChannel] Got only sample rate without audio data")
                            sample_rate = result
                        else:
                            print(f"[IndexTTS Pro MultiChannel] Unsupported result type: {type(result)}")
                            print(f"[IndexTTS Pro MultiChannel] Result value: {str(result)[:100]}")
                    
                    
                except Exception as e:
                    print(f"[IndexTTS Pro MultiChannel] Error generating {role} voice: {e}")
                    continue
            
            # 计算总时长
            total_duration = current_time
            print(f"[IndexTTS Pro MultiChannel] Total audio duration: {total_duration:.2f} seconds")
            
            # 为每个角色创建同步的音频轨道
            role_combined_audios = {}
            for role in ["Narrator", "Character1", "Character2", "Character3", "Character4", "Character5"]:
                # 创建与总时长相同的静音音频
                full_duration_audio = self._create_silence(total_duration, sample_rate)
                
                # 获取该角色的所有音频片段
                role_segments = role_audio_segments[role]
                
                if role_segments:
                    # 将角色的音频片段放置在正确的时间位置
                    for segment_info in segment_timing:
                        if segment_info["role"] == role:
                            start_sample = int(segment_info["start"] * sample_rate)
                            end_sample = int(segment_info["end"] * sample_rate)
                            segment_length = end_sample - start_sample
                            
                            # 确保片段长度与音频数据匹配
                            audio_data = segment_info["audio_data"]
                            if audio_data.ndim > 1:
                                audio_data = audio_data[0] if audio_data.shape[0] == 1 else np.mean(audio_data, axis=0)
                            
                            # 如果长度不匹配，进行调整
                            if len(audio_data) > segment_length:
                                audio_data = audio_data[:segment_length]
                            elif len(audio_data) < segment_length:
                                # 用静音填充
                                padding = np.zeros(segment_length - len(audio_data))
                                audio_data = np.concatenate([audio_data, padding])
                            
                            # 将音频数据放置在正确位置
                            full_duration_audio[start_sample:end_sample] = audio_data
                
                role_combined_audios[role] = (full_duration_audio, sample_rate)
            
            # 连接所有音频片段作为主输出
            # 收集所有片段时间信息用于主输出
            all_segment_data = []
            for segment_info in segment_timing:
                all_segment_data.append(segment_info["audio_data"])
            
            if all_segment_data:
                final_audio = np.concatenate(all_segment_data, axis=0)
            else:
                raise ValueError("Failed to generate any audio segments")
            
            # 计算音频长度
            if final_audio.ndim > 1:
                audio_length = final_audio.shape[1] / sample_rate
            else:
                audio_length = len(final_audio) / sample_rate
                
            print(f"[IndexTTS Pro MultiChannel] Multi-channel generation complete, total length: {audio_length:.2f} seconds")
            print(f"[IndexTTS Pro MultiChannel] Final audio shape before processing: {final_audio.shape}, sample rate: {sample_rate}")
            
            # 转为ComfyUI格式 - 需要是3D格式: [batch, channels, samples]
            def convert_to_comfyui_format(audio_numpy, sample_rate):
                # 转换为PyTorch张量
                audio_tensor = torch.tensor(audio_numpy, dtype=torch.float32)
                print(f"[IndexTTS Pro MultiChannel] Audio tensor dimensions: {audio_tensor.dim()}")
                
                # 确保音频数据是3D张量 [batch, channels, samples]
                if audio_tensor.dim() == 1:
                    # [samples] -> [1, 1, samples]
                    audio_tensor = audio_tensor.unsqueeze(0).unsqueeze(0)
                    print(f"[IndexTTS Pro MultiChannel] 1D tensor reshaped to 3D: [1, 1, {audio_tensor.shape[-1]}]")
                elif audio_tensor.dim() == 2:
                    # [channels, samples] -> [1, channels, samples]
                    audio_tensor = audio_tensor.unsqueeze(0)
                    print(f"[IndexTTS Pro MultiChannel] 2D tensor reshaped to 3D: [1, {audio_tensor.shape[1]}, {audio_tensor.shape[2]}]")
                
                print(f"[IndexTTS Pro MultiChannel] Final tensor shape: {audio_tensor.shape}")
                return {"waveform": audio_tensor, "sample_rate": sample_rate}
            
            # 转换主输出音频
            combined_audio_output = convert_to_comfyui_format(final_audio, sample_rate)
            
            # 转换每个角色的音频
            narrator_audio_output = convert_to_comfyui_format(role_combined_audios["Narrator"][0], sample_rate)
            character1_audio_output = convert_to_comfyui_format(role_combined_audios["Character1"][0], sample_rate)
            character2_audio_output = convert_to_comfyui_format(role_combined_audios["Character2"][0], sample_rate)
            character3_audio_output = convert_to_comfyui_format(role_combined_audios["Character3"][0], sample_rate)
            character4_audio_output = convert_to_comfyui_format(role_combined_audios["Character4"][0], sample_rate)
            character5_audio_output = convert_to_comfyui_format(role_combined_audios["Character5"][0], sample_rate)
            
            # 生成SubtitleJSON字符串
            subtitle_json = json.dumps(subtitle_data, ensure_ascii=False, indent=2)
            print(f"[IndexTTS Pro MultiChannel] Generated subtitle data with {len(subtitle_data)} items")
            ...
            # 连接为字符串
            simplified_subtitle_str = "\n".join(simplified_subtitles)
            print(f"[IndexTTS Pro MultiChannel] Generated simplified subtitle format with processed sentences")
            
            # 最终返回ComfyUI格式的音频数据、种子、JSON字幕和简化字幕
            return (
                combined_audio_output,  # combined_audio
                narrator_audio_output,  # narrator_audio_out
                character1_audio_output,  # character1_audio_out
                character2_audio_output,  # character2_audio_out
                character3_audio_output,  # character3_audio_out
                character4_audio_output,  # character4_audio_out
                character5_audio_output,  # character5_audio_out
                seed,  # seed
                subtitle_json,  # Subtitle
                simplified_subtitle_str  # SimplifiedSubtitle
            )
            
        except Exception as e:
            import traceback
            print(f"[IndexTTS Pro MultiChannel] Generation failed: {e}")
            print(traceback.format_exc())
            raise RuntimeError(f"Multi-channel generation failed: {e}")